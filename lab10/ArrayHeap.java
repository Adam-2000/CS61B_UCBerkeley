...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (getNode(parentIdx).priority() > getNode(index).priority()) {
    ...
    private void sink(int index) {
        ...
        if (getNode(leftIdx).priority() < currentNode.priority()) {
            minIdx = leftIdx;
        }
        int rightIdx = rightIndex(index);
        if (inBounds(rightIdx)) {
            if (getNode(rightIdx).priority() < getNode(minIdx).priority()) {
                minIdx = rightIdx;
            }
        }
        if (minIdx != index) {
    ...
    public T removeMin() {
        ...
        T retItem = contents[1].item();
        if (--size > 0) {
            contents[1] = contents[size + 1];
            sink(1);
        }
        return retItem;
    }
